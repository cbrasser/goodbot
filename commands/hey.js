...
exports.run = async (client, message, args) => {
    ...
    /*
        if (this.cmdProps.np) {
            ...
            message.channel.createMessage({embed: {title: 'Now Playing...', description: np}})
        } else if (this.cmdProps.message) {
            message.channel.createMessage(this.cmdProps.message)
        } else {
            message.addReaction(this.cmdP   rops.reaction)
        }
        ing()
            } else {
    ...
    */
    ...
    conn.playFile('files/carlos.mp3', streamOptions);
